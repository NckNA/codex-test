...
| Safe reference mutation contract | Repository create/update methods and owner/admin RLS exist, but stale/idempotency/audit/deactivation lifecycle is not hardened; DB hard delete remains possible | Separate MacDent reference-admin API not identified in bounded loaded-module search | `PARTIAL` | `HARDEN` | `LAB-WORK-REFERENCE-ADMIN-RECON-002B` |
| Owner/admin reference-management UI | No production management surface in current Settings page | Separate MacDent management surface not proven by current evidence | `PARTIAL` | `EXTEND` | Only after 002B + hardened mutation foundation |
| Historical inactive reference preservation | Desired semantic contract identified; current reference lifecycle is not fully hardened | Not proven in bounded MacDent evidence | `PARTIAL` | `HARDEN` | 002B must define deactivate/reactivate and hard-delete prohibition |
